...
if [ "$bmi_option" = "personalized" ]; then 
    apptainer exec --fakeroot --writable-tmpfs /mimer/NOBACKUP/groups/snic2022-22-770/Azilis_workspace/singularity_image/smplify_bmi_options.sif \
    bash -c '
    cd /workspace/smplify-x/
    python3.8 smplifyx/main.py --config cfg_files/fit_smplx.yaml --data_folder '$folder_path' --output_folder '$folder_path' --visualize="False" --model_folder models/ --vposer_ckpt ../vposer_v1_0/ --part_segm_fn smplx_parts_segm.pkl --use_vposer True --gender male --bmi="personalized"
    '
elif [ "$bmi_option" = "fixed" ]; then 
    apptainer exec --fakeroot --writable-tmpfs /mimer/NOBACKUP/groups/snic2022-22-770/Azilis_workspace/singularity_image/smplify_bmi_options.sif \
    bash -c '
    cd /workspace/smplify-x/
    python3.8 smplifyx/main.py --config cfg_files/fit_smplx.yaml --data_folder '$folder_path' --output_folder '$folder_path' --visualize="False" --model_folder models/ --vposer_ckpt ../vposer_v1_0/ --part_segm_fn smplx_parts_segm.pkl --use_vposer True --gender male --bmi="fixed" --fixedHeight='$height' --fixedWeight='$weight'
    '
elif [ "$bmi_option" = "free" ]; then 
    apptainer exec --fakeroot --writable-tmpfs /mimer/NOBACKUP/groups/snic2022-22-770/Azilis_workspace/singularity_image/smplify_bmi_options.sif \
    bash -c '
    cd /workspace/smplify-x/
    python3.8 smplifyx/main.py --config cfg_files/fit_smplx.yaml --data_folder '$folder_path' --output_folder '$folder_path' --visualize="False" --model_folder models/ --vposer_ckpt ../vposer_v1_0/ --part_segm_fn smplx_parts_segm.pkl --use_vposer True --gender male --bmi="free"
    '
else
    echo "Error: Choose 'free', 'fixed' or 'personalized' for the -b option"
fi
